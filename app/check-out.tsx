...
export default function CheckOutScreen() {
  ...
  const handleCheckOut = async (values: typeof location) => {
    setIsLoading(true);
    setError('');

    try {
      const response = await axios.post(
        getFullURL(API_ENDPOINTS.ATTENDANCE.CHECKOUT),
        {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Check out gagal');
      }
    } catch (err: any) {
      setError(
      ...
      <View style={styles.content}>
        <View style={styles.headerContainer}>
          <FontAwesome
            ...
            style={styles.icon}
          />
          <Text style={[styles.title, { color: textColor }]}>Check Out</Text>
